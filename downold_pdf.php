...
if (isset($_GET['id_cand'])) {
    ...
    $sql = "SELECT * FROM user WHERE user_id=$searchValue";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            $pdf->Cell(0, 10, $row['user_full_name'], 0, 1, 'L');

            $pdf->Cell(40, 10, 'Email:', 0, 0, 'L');
            $pdf->Cell(0, 10, $row['user_email'], 0, 1, 'L');

            $pdf->Cell(40, 10, 'Téléphone:', 0, 0, 'L');
            $pdf->Cell(0, 10, $row['user_whatsup'], 0, 1, 'L');
            $pdf->Cell(40, 10, 'Date de création:', 0, 0, 'L');
            $pdf->Cell(0, 10, $row['Date_Creation'], 0, 1, 'L');
